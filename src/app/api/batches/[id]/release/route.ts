...
import { releaseBatch } from "@/lib/order-service";
...
export const POST = withPermission("BATCH", "EXECUTE", async ({ req, actor }, context) => {
  const id = await getId(context);
  const input = await readJson(req, releaseSchema);
  return ok(await releaseBatch(id, input, actor, getClientIp(req)));
});
